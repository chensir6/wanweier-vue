    ...
    <div class="section-one-nav">
      ...
      <ul>
        <li v-for="(item, index) in cinfoPages[this.page - 1]" :key="index">
          ...
          <div class="introduce">
            <div class="detail">
              <span class="local">{{ item.name }}</span>
              <div class="rank">
                <span class="container">{{ item.label1 }}</span>
...
.position .more1 ul:hover .more1 {
  display: visible;
}
.position .more1 ul li {
  display: block;
  margin-top: 5px;
}
.position ul li a {
...
.section-one-nav ul li:hover .introduce {
  transform: translateY(-140px);
  background: rgba(102, 102, 102, 0.6);
  color: #fff;
}
.section-one-nav ul li:hover .rank {
  display: none;
}
.section-one-nav ul li:hover {
  background: rgba(102, 102, 102, 0.6);
}
